...
import { setVendorNotes } from "@/lib/actions/vendors";
import { AI_NAME } from "@/lib/ai-name";

/** Editable, persisted owner notes on a vendor. Mirrors SubNotes. */
export function VendorNotes({ slug, notes }: { slug: string; notes: string }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        await setVendorNotes(slug, text);
        setSaved(text);
      } catch {
        setError(true);
      }
  ...
  return (
    <Card className="max-w-[680px] p-3.5">
      <div className="mb-2 flex items-center">
        ...
        {!dirty && !pending && saved && <span className="font-mono text-[10px] text-money">saved</span>}
      </div>
      <textarea
        value={text}
        onChange={(e) => setText(e.target.value)}
        rows={6}
        placeholder={`What the ${AI_NAME} (and you) should remember about this vendor — pricing, lead times, quirks…`}
      ...
      />
      {error && <div className="mt-1.5 text-[11px] text-flag">Couldn&apos;t save — try again.</div>}
