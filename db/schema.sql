...
-- ─── Floor-plan designer (docs/floor-plan-designer-plan.md §15) ─────────────
-- Live, editable designs — one jsonb PlanDoc each (lib/plan-doc.ts), saved
-- whole with a rev counter. Project- or lead-scoped like estimates; templates
-- carry neither. Runner: db/apply-floor-designer.mjs.
...
-- Immutable snapshots ("Save version"); publishing and printing cut from these.
...
-- 3D captures, rendered sheet PDFs, underlays, and photo pins are files rows
-- indexed per design.
...
-- Comments pinned to a point / item (owner in the designer, client via portal).
CREATE TABLE IF NOT EXISTS plan_design_comments (
  id          bigserial PRIMARY KEY,
  design_id   bigint NOT NULL REFERENCES plan_designs(id) ON DELETE CASCADE,
  version_id  bigint REFERENCES plan_design_versions(id) ON DELETE SET NULL,
  anchor      jsonb NOT NULL,                      -- { levelId, x, y, itemId? }
...
-- A published floor-plan version remembers the design + snapshot it was cut
-- from, and the 3D preview PNG shown on its card.
...
-- Catalog items become placeable: physical size, placement kind (a PlaceKind
-- from lib/plan-doc.ts or ''), a parsed price (price stays the display string),
-- an optional install labour cost item, a glTF model key, and a 3D material.
...
-- Measure (lib/plan-measures.ts key, optionally per material tag) → cost-book
-- item, so a plan generates estimate lines through addTakeoffLines().
