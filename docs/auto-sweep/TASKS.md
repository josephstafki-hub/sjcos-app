...
### P1-D · Team Chat
- [~] **P1-D1** Remove hardcoded channels; add ability to **create and remove channels**. Team members, subs, and AI models can each be added to channels **independently**. AI models only respond when invoked via `@model_name`. — DONE: channel create/remove, independent sub + AI membership, `@model_name`-gated AI. REMAINING: independent **team-member** add — no internal-staff roster exists in the schema (users are owner/sub/client; the only internal human is the owner, implicit everywhere). See PROGRESS 2026-07-17.
