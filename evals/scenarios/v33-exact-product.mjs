...
const scenario = {
  ...
  checks: [
    ...
    { id: "no-board-or-selection", mandatory: true, describe: "the exact faucet is not turned into a selection or mood-board choice (other undecided scopes may still get their own path)", check: async (f) => { const sels = await f.run(`SELECT area, choice FROM project_selections WHERE project_id = $1 AND (area ILIKE '%faucet%' OR area ILIKE '%trinsic%' OR area ILIKE '%9159%')`, [f.projectId]); const opts = await f.run(`SELECT o.name FROM project_selection_options o JOIN project_selections s ON s.id = o.selection_id WHERE s.project_id = $1 AND (o.name ILIKE '%trinsic%' OR o.sku ILIKE '%9159%' OR o.name ILIKE '%9159%')`, [f.projectId]); const boards = await q.count(f.run, `SELECT 1 FROM project_mood_boards WHERE project_id = $1 AND (room ILIKE '%faucet%' OR title ILIKE '%faucet%')`, [f.projectId]); return { pass: sels.length === 0 && opts.length === 0 && boards === 0, detail: `faucet_selections=${sels.length} faucet_options=${opts.length} faucet_boards=${boards}` }; } },
    { id: "no-other-selection-work", mandatory: false, describe: "did not open selection work for unrelated scopes on a single product email", check: async (f, ex, r) => { const used = toolsIn(r?.trace).filter((t) => ["create_mood_board", "create_selection_item", "build_selection_plan", "add_selection_option"].includes(t)); const sels = await q.count(f.run, `SELECT 1 FROM project_selections WHERE project_id = $1`, [f.projectId]); return { pass: used.length === 0 && sels === 0, detail: `tools=${used.join(",") || "none"} selections=${sels}` }; } },
