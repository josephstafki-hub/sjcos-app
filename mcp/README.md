...
live, plus `scope_change_path` — `precon_estimate` or `change_order` — which
says what a client change becomes on this job **right now**.

| Tool | What it does |
|---|---|
| `list_project_estimates` | Every estimate with its lines, the change orders and document drafts, `formal_estimate_id` and `scope_change_path`. Read-only |
| `create_estimate` | A NEW estimate (`project_slug`, `title`, `kind?` default `formal`, `rail?`). A `precon_change` is refused on a job under contract; a second `formal` is allowed but flagged in the result |
| `add_estimate_lines` | Add lines to an estimate, any status (description, section, unit, qty, `unit_cost_cents`, `markup_pct?`); totals recomputed; the result says if the client already saw an older total or a PDF needs regenerating |

`create_document_draft` REQUIRES the record a template is generated from and
